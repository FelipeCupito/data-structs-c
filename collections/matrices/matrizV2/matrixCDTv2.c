...
struct tFila {
	...
	elementType max; // MÃ¡ximo elemento de la fila
	int maxCol; // Columna del mÃ¡ximo elemento de la fila
};

static void calculateRowMax(struct tFila *);
...
setElement(matrixADT m, size_t row, size_t col, elementType elem) {
	...
	if(row >= rows(m)) {
		...
		m->dim = row + 1;
		// Seteo el mÃ¡ximo de la fila
		m->filas[row].max = elem;
		m->filas[row].maxCol = col;
	}
	...
	// Actualizo el mÃ¡ximo de la fila
	if(m->filas[row].maxCol == col) {
		// Si se pisa el elemento mÃ¡ximo de la fila, el mÃ¡ximo debe recalcularse.
		calculateRowMax(&m->filas[row]);
	} else if(compare(elem, m->filas[row].max) > 0) { 
		// Si elem es mayor que el mÃ¡ximo de la fila
		m->filas[row].max = elem;
	}
	return 1;	
}
...
elementType * 
maxElements(matrixADT m, elementType min) {
	elementType * res = malloc(rows(m) * sizeof(elementType));
	for(int i = 0; i < rows(m); i++) {
		if(m->filas[i].dim == 0) {
			res[i] = min;
		} else {
...
static void 
calculateRowMax(struct tFila * fila) {
	elementType max = fila->max;
	for(int i = 0; i < fila->dim; i++) {
		if(fila->occupied[i]) {
			if(compare(fila->elems[i], max) > 0) {
				max = fila->elems[i];
			}
		}
	}
...
main(void) {
	...
	int * max = maxElements(m, -1);
	for(int i = 0; i < rows(m); i++) { // [12, -1, 23]
		printf("%d ", max[i]);
	}
	freeMatrix(m);
}